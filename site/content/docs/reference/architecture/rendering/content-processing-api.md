...
| `page.href` | (automatic) | `{{ page.href }}` | URL with baseurl applied (for display in templates) |
| `page._path` | (automatic) | `{{ page._path }}` | Site-relative URL without baseurl (for comparisons and logic) |
| `page.title` | (from metadata or generated) | `{{ page.title }}` | Page title (humanized from filename if missing) |

**URL Pattern Strategy:**

- **`page.href`**: Primary property for display — automatically includes baseurl from site config
- **`page._path`**: Use for comparisons, menu activation, filtering (without baseurl)

This pattern is ergonomic: templates use `{{ page.href }}` for display and it "just works" for all deployment scenarios (GitHub Pages, Netlify, S3, file://, etc.). Use `{{ page._path }}` when you need to compare URLs without baseurl.
...
| `section.subsection_index_urls` | (automatic) | `{% if page._path not in section.subsection_index_urls %}` | Set of subsection index URLs (for nav de-duplication) |
...
| `section.href` | (automatic) | `{{ section.href }}` | Section URL with baseurl applied (for display) |
| `section._path` | (automatic) | `{{ section._path }}` | Site-relative URL without baseurl (for comparisons) |
