...
- If you were using `template_engine: jinja2` in your config, switch to `template_engine: kida`. Kida uses Jinja2-compatible syntax, so most templates work without changes. See the [Kida migration guide](/docs/build-sites/customize/templating/kida/migration/from-jinja/) for details.
...
- **Jinja2 removed**: If your config specifies `template_engine: jinja2`, change it to `kida`. See the [Kida migration guide](/docs/build-sites/customize/templating/kida/migration/from-jinja/).
