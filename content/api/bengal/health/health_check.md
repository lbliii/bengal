...
#### `run`
```python
def run(self, build_stats: dict | None = None, verbose: bool = False, profile: BuildProfile = None, incremental: bool = False, context: list[Path] | None = None, cache: Any = None, build_context: Any = None) -> HealthReport
```
...
| `build_context` | `Any` | - | Optional BuildContext with cached artifacts (e.g., knowledge graph) that validators can use to avoid redundant computation |
