...
class TestAsyncErrorHandling:
    ...
    async def test_async_exception_propagates(self, env: Environment) -> None:
        """Exception in async function propagates."""
        async def failing_func() -> str:
            await asyncio.sleep(0)
            raise ValueError("async error")
        
        try:
            tmpl = env.from_string("{{ func() }}")
            
            if hasattr(tmpl, "render_async"):
                with pytest.raises((ValueError, Exception)):
                    await tmpl.render_async(func=failing_func)
            else:
                pytest.skip("Async render not available")
        except Exception:
            pytest.skip("Async function calls not supported")
...
class TestAsyncConcurrency:
    ...
    async def test_concurrent_renders(self, env: Environment) -> None:
        ...
        if hasattr(tmpl, "render_async"):
            tasks = [
                tmpl.render_async(name=f"User{i}")
                for i in range(10)
            ]
    ...
    async def test_concurrent_renders_different_templates(self, env: Environment) -> None:
        ...
        tmpls = [
            env.from_string(f"Template {i}: {{{{ x }}}}")
            for i in range(5)
        ]
...
class TestAsyncGenerator:
    ...
    async def test_async_generator_in_for(self, env: Environment) -> None:
        ...
        tmpl = env.from_string("{% for x in gen %}{{ x }}{% endfor %}")
        
        try:
            if hasattr(tmpl, "render_async"):
                result = await tmpl.render_async(gen=async_range(3))
                assert "0" in result
                assert "1" in result
                assert "2" in result
            else:
            ...
            # Async generators may not be supported
            pytest.skip("Async generator iteration not supported")
