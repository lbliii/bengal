...
class TestDependencyTrackerBasics:
    ...
    def test_creates_with_cache_and_site(
...
class TestThreadSafety:
    ...
    def test_current_page_is_thread_local(
        ...
        threads = [
            threading.Thread(target=worker, args=(f"page{i}",))
            for i in range(3)
        ]
