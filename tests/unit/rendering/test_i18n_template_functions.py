...
def test_alternate_links(tmp_path: Path) -> None:
    ...
    en = Page(source_path=tmp_path / 'content' / 'en' / 'a.md', content='x', metadata={})
    fr = Page(source_path=tmp_path / 'content' / 'fr' / 'a.md', content='x', metadata={})
    en.lang = 'en'
    fr.lang = 'fr'
    ...
    engine = TemplateEngine(site)
    tmpl = engine.env.from_string("{% for l in alternate_links(page) %}{{ l.hreflang }}|{{ l.href }}\n{% endfor %}")
