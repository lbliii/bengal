...
class TestHighConcurrencyStress:
    ...
    def test_indexing_thread_safety_large_scale(self, tmp_path: Path):
        ...
        with patch("multiprocessing.cpu_count", return_value=16):
            ...
            with patch("bengal.autodoc.extractors.python.extractor.get_optimal_workers", return_value=16):
        ...
        for c_name in colliding_names:
            ...
            assert len(entries) == num_modules, f"Lost entries for {c_name}! Expected {num_modules}, got {len(entries)}"
            ...
            if "derived" in element.name:
                for child in element.children:
                    if child.element_type == "class":
                        ...
                        assert len(inherited_methods) == 1, f"Missing inherited method in {child.name}"
