...
def test_regular_pages_caching(tmp_path):
    ...
    assert site.page_cache._regular is not None
    assert len(site.page_cache._regular) == 2

    # Second access should use cache (same object reference)
    regular2 = site.regular_pages
    assert regular2 is site.page_cache._regular
...
def test_regular_pages_empty_site(tmp_path):
    ...
    regular = site.regular_pages
    assert regular == []
    assert site.page_cache._regular == []
...
def test_regular_pages_cache_performance(tmp_path):
    """Test that subsequent regular page access returns the cached collection."""
    site = Site(root_path=tmp_path)
    ...
    assert regular1 is site.page_cache._regular

    regular2 = site.regular_pages

    assert regular2 is regular1
    assert regular2 is site.page_cache._regular
...
def test_regular_pages_cache_across_multiple_accesses(tmp_path):
    ...
    for _ in range(10):
        ...
        assert regular is site.page_cache._regular
