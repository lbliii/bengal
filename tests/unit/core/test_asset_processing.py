...
""")
        ...
        # Check if jsmin is available by trying to import it
        try:
            import jsmin
            ...
            # jsmin not available - that's okay, minify() should handle it gracefully
            # Asset should still be marked as minified
            pass
...
class TestAssetFingerprinting:
    ...
    def test_uses_minified_content_for_hash(self, temp_asset_dir):
        ...
        # Verify it's different from source hash
        asset2 = Asset(source_path=css_file)
        source_hash = asset2.hash()
        
        # Hashes might be same if minification doesn't change content much
        # But fingerprint should still be set correctly
