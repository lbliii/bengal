...
class TestBuildCommandHelpers:
    ...
    def test_should_regenerate_autodoc_from_config_enabled(self, tmp_path):
        ...
        with patch('bengal.cli.commands.build._check_autodoc_needs_regeneration', return_value=True):
            result = _should_regenerate_autodoc(
                autodoc_flag=None,
                config_path=config_path,
                root_path=tmp_path,
...
class TestUtilsCommands:
    ...
    def test_theme_list_command(self, tmp_path):
        ...
        runner = CliRunner()
        with runner.isolated_filesystem(temp_dir=tmp_path):
            from bengal.cli.commands.theme import theme_list
            
            result = runner.invoke(theme_list, catch_exceptions=False)
