...
class TestExternalRefValidatorCodes:
    ...
    def test_code_is_in_h7xx_range(self, mock_site):
        ...
        for result in results:
            if result.code:
                assert result.code.startswith("H7"), (
                    f"Expected H7xx code, got {result.code}"
                )
...
class TestExternalRefValidatorBehavior:
    ...
    def test_reports_unresolved_refs(self, mock_site):
        ...
        warning_results = [r for r in results if r.status in (CheckStatus.WARNING, CheckStatus.ERROR)]
