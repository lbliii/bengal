...
def mock_site():
    ...
    pages = [
        Page(
            ...
            content="Post 1",
            metadata={"title": "Post 1", "tags": ["python", "testing"], "date": datetime(2024, 1, 1)}
        ),
        Page(
            source_path=Path("/fake/site/content/post2.md"),
            content="Post 2",
            metadata={"title": "Post 2", "tags": ["python", "django"], "date": datetime(2024, 1, 2)}
        ),
        Page(
...
class TestConditionalGeneration:
    ...
    def test_generate_dynamic_pages_for_specific_tags(self, orchestrator, mock_site):
        ...
        generated = [p for p in mock_site.pages if p.metadata.get('_generated') and p.metadata.get('type') == 'tag']
...
class TestPerformanceOptimization:
    ...
    def test_selective_generation_calls_create_once_per_tag(self, orchestrator, mock_site):
        ...
        with patch.object(orchestrator, '_create_tag_pages', wraps=orchestrator._create_tag_pages) as mock_create:
            affected_tags = {'python'}
            orchestrator.generate_dynamic_pages_for_tags(affected_tags)
