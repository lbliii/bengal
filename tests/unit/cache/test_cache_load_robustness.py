...
'''
        ...
        assert "fingerprints=1" in result.stdout, (
            "Cache fingerprints not loaded in subprocess"
        )
...
class TestCacheExceptionHandling:
    ...
    def test_corrupted_compressed_cache_returns_empty(self, tmp_path: Path) -> None:
        ...
        assert len(loaded.file_fingerprints) == 0, (
            "Corrupted cache should return empty BuildCache"
        )
...
class TestCacheChangeDetection:
    ...
    def test_is_changed_detects_unchanged_file_from_loaded_cache(
        self, tmp_path: Path
    ) -> None:
        """
    ...
    def test_is_changed_detects_modified_file_from_loaded_cache(
        self, tmp_path: Path
    ) -> None:
        """
