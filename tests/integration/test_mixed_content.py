...
class TestMixedContent:
    ...
    def test_all_pages_discovered(self, built_site) -> None:
        ...
        assert len(built_site.pages) >= 7, (
            f"Expected at least 7 pages, found {len(built_site.pages)}"
        )

    def test_docs_section_exists(self, built_site) -> None:
        """Docs section should be discovered."""
        docs = [p for p in built_site.pages if "/docs/" in str(p.source_path)]
        assert len(docs) >= 2, f"Expected at least 2 docs pages, found {len(docs)}"

    def test_blog_section_exists(self, built_site) -> None:
        """Blog section should be discovered."""
        posts = [p for p in built_site.pages if "/blog/" in str(p.source_path)]
        assert len(posts) >= 2, f"Expected at least 2 blog pages, found {len(posts)}"

    def test_projects_section_exists(self, built_site) -> None:
        """Projects section should be discovered."""
        projects = [p for p in built_site.pages if "/projects/" in str(p.source_path)]
        assert len(projects) >= 2, f"Expected at least 2 project pages, found {len(projects)}"

    def test_docs_pages_have_doc_type(self, built_site) -> None:
    ...
    def test_blog_pages_have_blog_type(self, built_site) -> None:
        """Blog pages should have 'blog' content type from cascade."""
        posts = [p for p in built_site.pages if "/blog/post" in str(p.source_path)]
    ...
    def test_project_pages_have_portfolio_type(self, built_site) -> None:
        """Project pages should have 'portfolio' content type from cascade."""
        projects = [p for p in built_site.pages if "/projects/project" in str(p.source_path)]
    ...
    def test_site_builds_successfully(self, built_site) -> None:
    ...
    def test_all_sections_rendered(self, built_site) -> None:
    ...
    def test_sitemap_includes_all_sections(self, built_site) -> None:
        """Sitemap should include pages from all sections."""
        sitemap_path = built_site.output_dir / "sitemap.xml"
