...
- [x] `page.ast` returns typed `list[ASTNode]`
- [x] `page.html` renders from AST (with fallback to legacy)
- [x] `RawHTMLNode` handles directive output correctly
- [x] `walk_ast()` utility implemented and tested
- [x] TOC extraction uses AST walk (via `extract_toc_from_ast`)
- [x] Link transformation uses AST manipulation (via `ast_transforms.py`)
- [x] `PageContentMixin.plain_text` uses AST extraction when available
- [x] All existing tests pass
- [x] No template changes required
- [ ] Deprecation warning on `parsed_ast` access (deferred - would break internal pipeline)
