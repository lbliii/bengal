...
- Post-processing prints a hardcoded emoji header via `print()`/`reporter.log()` instead of `CLIOutput`:
  - `bengal/orchestration/postprocess.py:118-123`
...
- Stats display uses emoji headings and emoji-based “performance grade” tokens:
  - `bengal/orchestration/stats/display.py:186-188` (📊 Content Statistics)
  - `bengal/orchestration/stats/display.py:254-264` (🚀/⚡/🐌)
  - `bengal/orchestration/stats/display.py:321-327` (📈 Throughput)
  - `bengal/orchestration/stats/display.py:332` (📂 Output)
- Rich build summary has additional emoji headings:
  - `bengal/orchestration/summary.py:388-438` (Build Complete + 📂 Output)
...
- `bengal/orchestration/stats/display.py` (Content Statistics, Build Configuration, Performance, Throughput, Output)
- `bengal/orchestration/summary.py` (rich summary footer headings)
...
- **Evidence (40/40)**: Strong code evidence of current output sources and theme tokens:
  - `bengal/orchestration/stats/display.py:186-333`
  - `bengal/orchestration/postprocess.py:118-123`
  - `bengal/output/core.py:243-334`
  - `bengal/utils/rich_console.py:21-85`
- **Consistency (30/30)**: Multiple call sites confirm the same pattern (some centralized, some bypassing).
- **Recency (15/15)**: Files appear actively used in current builds (consistent with observed build output).
