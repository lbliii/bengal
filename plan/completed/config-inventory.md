...
| `THEME_DEV` | `--theme-dev` or `--profile theme-dev` | + rendering, navigation, menu | ✅ | ❌ |
| `DEVELOPER` | `--dev` or `--debug` or `--profile dev` | ALL | ✅ | ✅ |
...
**Note**: `--verbose` does NOT change the profile. It only controls output verbosity (shows phase timing, build stats). Use `--theme-dev` or `--dev` for profile-based health checks and metrics.
...
1. **✅ Add missing sections to `KNOWN_SECTIONS`** - DONE
   - Added: `search`, `content`, `autodoc`, `i18n`, `graph`
   - See: `bengal/config/loader.py:67-94`

2. **✅ Fix `max_workers` defaults** - DONE
   - Created `bengal/config/defaults.py` with `get_max_workers()` function
   - Auto-detects CPU count (leaves 1 core, minimum 4 workers)
   - Updated all code files to use `get_max_workers()`:
     - `render.py`, `taxonomy.py`, `asset.py`, `related_posts.py`, `content_discovery.py`
   - See: `bengal/config/defaults.py:39-61`

3. **✅ Standardize bool/dict config handling** - DONE
   - Created `normalize_bool_or_dict()` function
   - Handles: `health_check`, `search`, `graph`, `output_formats`
   - See: `bengal/config/defaults.py:345-407`

4. **✅ Create `bengal/config/defaults.py`** - DONE
   - Centralized `DEFAULTS` dictionary with all config defaults
   - Helper functions: `get_max_workers()`, `get_pagination_per_page()`, `is_feature_enabled()`, `get_feature_config()`
   - See: `bengal/config/defaults.py`

5. **✅ Deprecate duplicate keys** - DONE
   - Deprecation warnings implemented in `bengal/config/deprecation.py`
   - Tracks: `minify_assets`, `optimize_assets`, `fingerprint_assets`, `generate_sitemap`, `generate_rss`, `markdown_engine`
   - Warnings shown via `ConfigLoader.print_warnings()`
   - See: `bengal/config/deprecation.py:25-59`

### Remaining Recommendations

6. **Document profile behavior** - PARTIALLY DONE
   - ✅ `--verbose` behavior clarified: does NOT change profile, only controls output verbosity
   - ✅ `--quiet` flag exists (minimal output)
   - ⚠️ Profile documentation could be more prominent in user docs
   - See: `bengal/cli/commands/build.py:105-109` and `bengal/utils/profile.py:88-89`
