...
- **98 exception catches** across 37 files — many don't enrich with Bengal errors
...
**Candidate location**: `pipeline/core.py` for multi-page preprocessing

```python
# Current: Individual page errors logged without aggregation
for page in pages:
    try:
...
**98 exception catches** across 37 files. Many catch and log without using Bengal error patterns.
...
**File**: `pipeline/core.py` — Add aggregation for preprocessing errors

```python
from bengal.errors import ErrorAggregator, ErrorCode

def _preprocess_pages(self, pages: list[Page]) -> None:
    """Pre-process multiple pages with error aggregation."""
    aggregator = ErrorAggregator(total_items=len(pages))
    threshold = 5

    for page in pages:
        try:
            self._preprocess_content(page)
        except Exception as e:
            context = {
                "source_path": str(page.source_path),
                "error": truncate_error(e),
