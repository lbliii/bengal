...
**Impact**:
- ✅ Single source of truth for utility functions (DRY principle)
- ✅ Reduced cognitive load for contributors
- ✅ Better error debugging via specific exceptions
- ✅ Consistent codebase patterns
- ⚠️ Requires import updates across affected files
...
| File | Lines | Status |
|------|-------|--------|
| `bengal/utils/text.py:101` | 34 | ✅ Canonical (well-documented, full-featured) |
| `bengal/rendering/template_functions/strings.py:166` | 16 | ✅ Wrapper (delegates to utils.text) |
| `bengal/postprocess/output_formats/utils.py:19` | 22 | ❌ **Duplicate** (reimplements same logic) |
...
|------|----------------|--------|
| `bengal/utils/text.py:244` | Word-based truncation | ✅ Canonical |
| `bengal/postprocess/output_formats/utils.py:43` | Character-based truncation | ❌ **Different behavior** |
...
├── __init__.py              # DirectiveValidator (orchestrator, ~200 lines)
│   └── class DirectiveValidator:
│       - validate()
│       - _analyze_directives()
│
├── syntax.py                # Syntax validation (~250 lines)
│   └── class DirectiveSyntaxChecker:
│       - check_syntax()
│       - _validate_fence_structure()
│       - _validate_directive_names()
│
├── completeness.py          # Completeness validation (~250 lines)
│   └── class DirectiveCompletenessChecker:
│       - check_completeness()
│       - _check_required_options()
│       - _check_tab_markers()
│
├── performance.py           # Performance warnings (~200 lines)
│   └── class DirectivePerformanceChecker:
│       - check_performance()
│       - _check_nesting_depth()
│       - _check_directive_count()
│
└── rendering.py             # Output validation (~200 lines)
    └── class DirectiveRenderingChecker:
        - check_rendering()
...
├── __init__.py              # CLI commands (entry points, ~150 lines)
│   └── @new.command() handlers
│
├── presets.py               # Preset definitions (~100 lines)
│   └── PRESETS = {...}
│
├── wizard.py                # Interactive wizard (~250 lines)
│   └── class SiteWizard:
│       - run()
│       - _prompt_site_type()
│       - _prompt_theme()
│       - _prompt_features()
│
├── config_generator.py      # Config file generation (~200 lines)
│   └── class ConfigGenerator:
│       - generate()
│       - _create_site_yaml()
│       - _create_build_yaml()
│
└── content_generator.py     # Content scaffolding (~250 lines)
...
**Rationale**:
- ✅ Duplicates verified via grep (exact matches)
- ✅ Line counts verified via wc -l
- ✅ Exception patterns verified via code review
- ✅ Proposed structure follows proven patterns (`output_formats/` package)
- ✅ Low risk (utility consolidation, no public API changes)
