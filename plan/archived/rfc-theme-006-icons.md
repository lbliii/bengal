...
> **Why Merged**: Investigation revealed icon system is already well-designed:
> - LRU caching, preloading (~86 icons)
> - `ICON_MAP` for semantic aliases
> - Accessibility support (`aria_label`, `aria-hidden`)
>
> Only gap was moving `ICON_MAP` to config. This is now included in RFC-001's `icons.aliases` section.  
