...
**Confidence**: 92% 🟢 (High - prior art reconciled, implementation plan ready)
...
3. Handles active trail marking automatically (via immutable overlay pattern)
4. Provides a cleaner template API than direct Section manipulation
5. **Native Fallback Support**: Integrates cross-version target resolution to eliminate 404s on version switch
...
5. **Version-Switching Latency**: Version selector must run fallback logic (Cascade) on the client or via expensive template calls.
...
### Shared Content complexity (`_shared/`)

Bengal's unique `_shared/` content feature requires that certain pages appear in **all** versions. Current templates handle this by coincidence (because `_shared` pages have `version: None`), but NavTree must explicitly weave these into every version-specific tree during the pre-computation phase.
...
3. **Version-Native**: Versioning is first-class, not bolted on. NavTree is the authority on where to land when switching versions.
4. **Template Simplicity**: Single function call, clean API
5. **Performance**: O(1) tree lookup, cached per version.
6. **Shared Content Integration**: `_shared/` content is woven into all version-specific trees at build time.
7. **Active Trail Overlay**: Use an immutable tree with a lightweight "Context" overlay for per-page active trail detection to preserve caching benefits.
...
│  • Active trail marked per-page via Overlay                      │
│  • **Authority for Fallback URLs** (get_target_url)              │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
...
class NavTree:
    ...
    def get_target_url(self, page: Page, target_version_id: str) -> str:
        """
        Get the best landing URL in a different version for a given page.

        Uses the established Fallback Cascade:
        1. Exact Page Match
        2. Section Index Match
        3. Version Root Match
        """
        # Implementation delegates to NavTreeCache lookup + version_url.py logic
        ...

    def with_active_trail(self, current_page: Page) -> NavTreeContext:
        """
        Return a Context overlay with active trail marked.

        This avoids deep-copying the immutable cached tree by providing
        a wrapper that handles 'is_current' and 'is_in_trail' checks.
        """
        return NavTreeContext(self, current_page)
    ...
    def build(cls, site: Site, version_id: str | None = None) -> NavTree:
        ...
        Handles:
        1. Version-aware section discovery
        2. _shared/ content injection
        3. Logical URL resolution (stripping _versions prefixes)
        """
        ...
...
class NavTreeContext:
    """
    Lightweight overlay for a cached NavTree that provides per-page state.
    """
    tree: NavTree
    current_page: Page

    @cached_property
    def active_trail_urls(self) -> set[str]:
        """URLs in the path to current_page."""
        return {p.url for p in self.current_page.ancestors} | {self.current_page.url}

    def is_active(self, node: NavNode) -> bool:
        return node.url in self.active_trail_urls

    def is_current(self, node: NavNode) -> bool:
        return node.url == self.current_page.url
...
def register(env: Environment, site: Site) -> None:
    ...
    def get_nav_tree(page: Page | None = None) -> NavTreeContext:
        """
        Get navigation tree context for the current page.

        Returns an overlay on top of the cached version-specific tree.
        """
        ...
        return tree # Fallback to raw tree if no page
...
| **Fallback Success** | Zero 404s on version switch | Unknown | Automated navigation audit |
| **Shared Content** | `_shared/` pages in every version | Manual/Inconsistent | Integration test check |
...
2. **Fallback tests**: Verify `get_target_url` returns valid URLs for missing pages.
3. **Shared tests**: Verify `_shared/docs/foo.md` appears in v1, v2, and latest trees.
4. **Performance benchmark**: Compare `Site.build()` time with 10 versions vs 1 version.
5. **Thread-safety test** for concurrent cache access
6. **Integration test** with versioned site (`tests/roots/test-versioned/`)
7. **Regression test** for existing `get_nav_tree()` callers
...
## Appendix: Version-Switching Logic (The Cascade)

NavTree will internalize the following logic from `version_url.py`:

1. **Exact Match**: If `target_version.nav_tree.find(page.logical_url)` exists.
2. **Section Index**: If `target_version.nav_tree.find(page.logical_parent_url)` exists.
3. **Root**: `target_version.nav_tree.root.url`.

This ensures that the version dropdown always uses a valid `href` computed at build time.
