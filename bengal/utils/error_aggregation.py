...
- **threshold=5**: Default for most batch operations (pages, assets)
- **threshold=10**: For operations with many items (100+)
- **threshold=3**: For critical operations where any error matters

**Error Context:**

The `extract_error_context()` function automatically extracts:
- Template name and line number (for TemplateRenderError)
- File paths (for items with source_path attribute)
- Suggestions (from BengalError exceptions)
...
def extract_error_context(error: Exception, item: Any | None = None) -> dict[str, Any]:
    ...
    if isinstance(error, AttributeError) and "suggestion" not in context:
        error_msg = str(error)
        suggestion = _get_attribute_error_suggestion(error_msg)
        if suggestion:
...
# Common attribute migrations and their suggestions
_ATTRIBUTE_MIGRATIONS: dict[str, tuple[str, str]] = {
    # Pattern: (old_attr_fragment, (suggested_replacement, explanation))
    "has no attribute 'url'": (".href", "Use .href for template URLs (baseurl included)"),
    "has no attribute 'relative_url'": ("._path", "Use ._path for internal path comparisons"),
    "has no attribute 'site_path'": ("._path", "Use ._path for internal path comparisons"),
    "has no attribute 'permalink'": (".href", "Use .href for template URLs"),
}


def _get_attribute_error_suggestion(error_msg: str) -> str | None:
    """
    Get actionable suggestion for common AttributeError patterns.

    Helps users migrate to the unified URL model (.href, ._path)
    and fix other common template attribute mistakes.

    Args:
        error_msg: The error message from AttributeError

    Returns:
        Suggestion string if pattern matches, None otherwise
    """
    for pattern, (replacement, explanation) in _ATTRIBUTE_MIGRATIONS.items():
        if pattern in error_msg:
            return f"Use {replacement} instead. {explanation}"
    return None
