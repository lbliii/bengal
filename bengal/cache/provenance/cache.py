...
class ProvenanceCache:
    ...
    _index: dict[str, ContentHash] = field(default_factory=dict)
    _records: dict[ContentHash, ProvenanceRecord] = field(default_factory=dict)
    _subvenance: dict[ContentHash, set[str]] = field(default_factory=dict)
    _loaded: bool = False
    _dirty: bool = False
    ...
    def _load_index(self) -> None:
        ...
        if index_path.exists():
            try:
                ...
                self._index = {k: ContentHash(v) for k, v in data.get("pages", {}).items()}
    ...
    def get(self, page_path: str) -> ProvenanceRecord | None:
    ...
    def is_fresh(self, page_path: str, current_provenance: Provenance) -> bool:
        """
    ...
    def get_affected_by_path(self, input_path: str, site_root: Path) -> set[str]:
        """
        ...
        """
        from bengal.cache.provenance.types import hash_file
    ...
    def gc(self, valid_pages: set[str]) -> int:
        """
