...
class ProvenanceFilter:
    ...
    def __init__(
        ...
        self._asset_hashes: dict[str, ContentHash] = {}
    ...
    def _load_asset_hashes(self) -> None:
        ...
        if asset_cache_path.exists():
            try:
                ...
                self._asset_hashes = {k: ContentHash(v) for k, v in data.items()}
    ...
    def _compute_provenance(self, page: Page) -> Provenance:
        ...
        """
        provenance = Provenance()
        rel_path = self._get_page_path(page)
        ...
        if not is_virtual and page.source_path.exists():
            ...
            if autodoc_source and page.metadata.get("is_autodoc"):
                ...
                if source_path.exists():
                    ...
                    provenance = provenance.with_input("autodoc_source", rel_source, source_hash)
            ...
            if tag_name:
                ...
                provenance = provenance.with_input("taxonomy", f"tag:{tag_name}", tag_hash)
            ...
            if cli_source and page.metadata.get("is_cli"):
                ...
                if source_path.exists():
                    source_hash = hash_file(source_path)
                    provenance = provenance.with_input("cli_source", str(source_path), source_hash)
        ...
        provenance = provenance.with_input("config", "site_config", self._config_hash)

        return provenance

    def _get_page_path(self, page: Page) -> str:
        """Get relative page path for cache key."""
        try:
            return str(page.source_path.relative_to(self.site.root_path))
        except ValueError:
    ...
    def _get_asset_path(self, asset: Asset) -> str:
        """Get relative asset path for cache key."""
        try:
            return str(asset.source_path.relative_to(self.site.root_path))
        except ValueError:
