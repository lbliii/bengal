...
from bengal.services.theme_manifest import read_theme_extends
...
def _read_theme_extends(site_root: Path, theme_name: str) -> str | None:
    ...
    if site_manifest.exists():
        try:
            return read_theme_extends(site_manifest)
    ...
    try:
        ...
        if pkg:
            ...
            if manifest_path and manifest_path.exists():
                try:
                    return read_theme_extends(manifest_path)
                except Exception as e:
                    emit(
                        None,
                        "debug",
    ...
    if bundled_manifest.exists():
        try:
            return read_theme_extends(bundled_manifest)
        except Exception as e:
            emit(
                None,
                "debug",
