...
class BuildOrchestrator:
    ...
    def build(
        ...
        self._phase_discovery(cli, incremental)

        # Phase 3: Cache Discovery Metadata
        self._phase_cache_metadata()

        # Phase 4: Config Check and Cleanup
        incremental, config_changed = self._phase_config_check(cli, cache, incremental)

        # Phase 5: Incremental Filtering (determine what to build)
        filter_result = self._phase_incremental_filter(
            cli, cache, incremental, verbose, build_start
        )
    ...
    def _phase_discovery(self, cli, incremental: bool) -> None:
        ...
        Side effects:
            - Populates self.site.pages with discovered pages
            - Populates self.site.sections with discovered sections
            - Updates self.stats.discovery_time_ms
        """
    ...
    def _phase_incremental_filter(
        ...
        Side effects:
            - Updates self.stats with cache hit/miss statistics
