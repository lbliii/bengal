...
@command_metadata(
    category="theming",
    description="List available theme features",
    examples=[
        "bengal utils theme features",
        "bengal utils theme features --category navigation",
        "bengal utils theme features --enabled",
    ],
    requires_site=False,
    tags=["theming", "info", "quick"],
)
...
@click.option("--category", "-c", help="Filter by category (navigation, content, search, etc.)")
@click.option("--enabled", is_flag=True, help="Show only enabled features for this site")
@click.option("--defaults", is_flag=True, help="Show only default features")
def features(source: str, category: str | None, enabled: bool, defaults: bool) -> None:
    ...
    # Get site config if available (for --enabled flag)
    enabled_features: set[str] = set()
    if enabled or source != ".":
        try:
    ...
    if category:
        # Allow partial category matching (e.g., "nav" matches "navigation")
        matching_categories = [cat for cat in by_category if cat.startswith(category.lower())]
        if not matching_categories:
            cli.warning(f"No features in category: {category}")
            cli.info(f"Available categories: {', '.join(sorted(by_category.keys()))}")
            return
        by_category = {cat: by_category[cat] for cat in matching_categories}

    # Filter to enabled only if requested
    if enabled:
        if not enabled_features:
            cli.info("No features enabled in current site config.")
            cli.info("\nTo enable features, add to bengal.toml:")
            cli.info('[theme]\nfeatures = ["navigation.toc", "content.code.copy"]')
            return
        by_category = {
            cat: [f for f in features_list if f.key in enabled_features]
            for cat, features_list in by_category.items()
        }
        by_category = {cat: fl for cat, fl in by_category.items() if fl}

    # Filter to defaults only if requested
    if defaults:
        by_category = {
            cat: [f for f in features_list if f.default]
            for cat, features_list in by_category.items()
        }
        by_category = {cat: fl for cat, fl in by_category.items() if fl}

    # Display
    cli.header("Available Theme Features")
    cli.info("")

    for cat_name in sorted(by_category.keys()):
        cat_features = by_category[cat_name]
        cli.info(f"[{cat_name.upper()}]")

        for feature in sorted(cat_features, key=lambda f: f.key):
            # Status indicators
            is_enabled = feature.key in enabled_features
            is_default = feature.key in default_keys

            status = "✅" if is_enabled else "  "
            default_marker = " (default)" if is_default else ""

            cli.info(f"  {status} {feature.key}{default_marker}")
            cli.info(f"      {feature.description}")
    ...
    num_enabled = len(enabled_features)

    cli.info("─" * 50)
    cli.info(f"Total: {total} features | Defaults: {num_defaults} | Enabled: {num_enabled}")
    cli.info("")
    cli.info("To enable features, add to bengal.toml:")
    cli.info('[theme]\nfeatures = ["navigation.toc", "content.code.copy"]')
