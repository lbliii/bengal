...
Phase 3.3: Added handle_blank_line_with_stack() which uses ContainerStack
for indent queries instead of explicit parameters.
"""
...
def handle_blank_line_with_stack(
    next_token: Token | None,
    containers: ContainerStack,
) -> BlankLineResult:
    """Handle blank line in list parsing using ContainerStack.

    Phase 3.3: Stack-aware version that queries indent values from the
    current frame instead of receiving them as parameters.
    ...
    match next_token.type:
        ...
        case TokenType.INDENTED_CODE:
            return _handle_blank_then_indented_code_with_stack(
                next_token,
                containers,
            )
...
def _handle_blank_then_indented_code_with_stack(
    token: Token,
    containers: ContainerStack,
) -> BlankLineResult:
    """Handle INDENTED_CODE token after blank line using ContainerStack.

    Phase 3.3: Stack-aware version.
